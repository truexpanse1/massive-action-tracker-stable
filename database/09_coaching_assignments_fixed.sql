...
-- Clients can mark notes as read (only is_read and read_at fields)
...
-- Managers can update their assignments (full control)
CREATE POLICY "Managers can update their assignments"
  ON coaching_assignments FOR UPDATE
  USING (auth.uid() = manager_id);

-- Clients can update status and completion notes (limited fields)
