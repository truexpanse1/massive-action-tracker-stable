-- Migration: Add Proposals System to MAT (CORRECTED)
-- Description: Adds service_packages and proposals tables with Dream Client Profile integration
-- Matches MAT's schema: UUID for companies/users, BIGINT for clients/hot_leads/buyer_avatars
...
CREATE TABLE IF NOT EXISTS proposals (
    ...
    -- Relationships (BIGINT for business data tables, UUID for service packages)
    hot_lead_id BIGINT REFERENCES hot_leads(id) ON DELETE SET NULL,
    avatar_id BIGINT REFERENCES buyer_avatars(id) ON DELETE SET NULL,
...
ON proposals FOR SELECT
USING (true); -- Public access for shareable proposal URLs
